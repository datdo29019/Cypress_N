context('Change password', () => {
  ...
  it('Change password', () => {
    ...
    cy.intercept('POST', 'https://apis-test.vnlp.ai/auth/change-password').as('false')
    cy.intercept('POST', 'https://apis-test.vnlp.ai/auth/change-password').as('false1')
    cy.intercept('POST', 'https://apis-test.vnlp.ai/auth/change-password').as('true')
    ...
    cy.get('form.ng-untouched > :nth-child(2) > .ng-untouched').should('have.attr', 'placeholder', 'New password')
...
//Kiểm tra khi passwork kh đủ 6 kí tự
    ...
    cy.wait('@false1').its('response.body.success').should('eq', false)
